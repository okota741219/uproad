...
/**
 * Servlet implementation class DataAddTestServlet
 */
@WebServlet("/DataAddTestServlet")
public class DataAddTestServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DataAddTestServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<UserBean> list = sb.Login();	
		
		// 結果のページをディスパッチする
		 if ( list != null && ! list.isEmpty()) {
		     dispatcher = request.getRequestDispatcher("LogCon.jsp");
		 }
		 else {
